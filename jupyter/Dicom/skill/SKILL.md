...
- If the user says their DICOM files have many/lots of frames, are multi-frame, or reports memory exhaustion/OOM while extracting frames, and the assembled pipeline contains both `DicomToImageV3` and `DicomDrawRegions` (`pixel_phi_builder`, `pixel_phi_zero_shot`, `pixel_remove_all_text`), wrap the final `PipelineModel` with `DicomPretrainedPipeline(pipeline)` from `sparkocr.pretrained` and call `.transform(dicom_df)` on that wrapper instead of `pipeline.transform(dicom_df)`. Read `references/stages.md` `DicomPretrainedPipeline` section first for the mechanism, requirements, and the trailing-stage caveat before generating this.
